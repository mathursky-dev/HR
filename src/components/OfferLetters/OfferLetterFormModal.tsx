...
export const OfferLetterFormModal: React.FC<OfferLetterFormModalProps> = ({
  ...
  // Breakup components
  const [basicSalary, setBasicSalary] = useState<number>(editingOffer?.compensationBreakup.basicSalary || 15000);
  const [hra, setHra] = useState<number>(editingOffer?.compensationBreakup.hra || 7500);
  const [specialAllowance, setSpecialAllowance] = useState<number>(editingOffer?.compensationBreakup.specialAllowance || 7500);
  const [performanceBonus, setPerformanceBonus] = useState<number>(editingOffer?.compensationBreakup.performanceBonus || 0);
