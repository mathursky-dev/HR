...
export const Sidebar: React.FC<SidebarProps> = ({ activeNav, onNavigate, onSelectNav }) => {
  const { followUps, candidates, interviews, theme, toggleTheme } = useRecruitment();
  ...
  const navSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'audit', label: 'Audit Trail', icon: '🛡️', isEmoji: true },
      ],
    },
  ];
  ...
  return (
    <aside className="w-56 bg-[#0F172A] text-slate-300 flex flex-col shrink-0 min-h-[calc(100vh-3.5rem)] select-none border-r border-slate-800">
      ...
      <div className="p-3 border-t border-slate-700 flex items-center gap-2.5 bg-[#0F172A]">
        <div className="w-7 h-7 rounded-full bg-slate-600 flex items-center justify-center text-[10px] font-bold text-white shrink-0">
          ES
        </div>
        <div className="overflow-hidden">
          <div className="text-[11px] font-bold text-white leading-tight truncate">Management Panel</div>
          <div className="text-[10px] opacity-60 truncate">Director / Super Admin</div>
        </div>
      </div>
